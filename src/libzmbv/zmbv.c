...
/* generate functions */
ZMBV_POSSIBLE_BLOCK_TPL(int8_t,  8)
ZMBV_POSSIBLE_BLOCK_TPL(int16_t,16)
ZMBV_POSSIBLE_BLOCK_TPL(int32_t,32)

ZMBV_COMPARE_BLOCK_TPL(int8_t,  8)
ZMBV_COMPARE_BLOCK_TPL(int16_t,16)
ZMBV_COMPARE_BLOCK_TPL(int32_t,32)

ZMBV_ADD_XOR_BLOCK_TPL(int8_t,  8)
ZMBV_ADD_XOR_BLOCK_TPL(int16_t,16)
ZMBV_ADD_XOR_BLOCK_TPL(int32_t,32)

ZMBV_ADD_XOR_FRAME_TPL(int8_t,  8)
ZMBV_ADD_XOR_FRAME_TPL(int16_t,16)
ZMBV_ADD_XOR_FRAME_TPL(int32_t,32)
  ...
  *firstByte = 0; /* frame flags */
  ...
  if (flags&ZMBV_PREP_FLAG_KEYFRAME) {
    ...
    *firstByte |= FRAME_MASK_KEYFRAME;
    ...
    header->compression = (!(zc->init_flags&ZMBV_INIT_FLAG_NOZLIB) ? COMPRESSION_ZLIB : COMPRESSION_NONE);
    ...
    /* restart deflate */
    if (!(zc->init_flags&ZMBV_INIT_FLAG_NOZLIB)) deflateReset(&zc->zstream);
  } else {
...
int zmvb_encode_finish_frame (zmbv_codec_t zc) {
  if (zc != NULL) {
    ...
    if (!(zc->init_flags&ZMBV_INIT_FLAG_NOZLIB)) {
      ...
      /* just spit out uncompressed frame */
...
extern int zmbv_decode_frame (zmbv_codec_t zc, void *framedata, int size) {
  if (zc != NULL && framedata != NULL && size > 1) {
    uint8_t tag;
    uint8_t *data = (uint8_t *)framedata;
    tag = *data++;
    if (--size <= 0) return -1;
    if (tag&FRAME_MASK_KEYFRAME) {
      zmbv_keyframe_header_t *header = (zmbv_keyframe_header_t *)data;
