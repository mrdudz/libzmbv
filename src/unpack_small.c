...
////////////////////////////////////////////////////////////////////////////////
static int next_screen (zmbvu_unpacker_t zc) {
  if (frameno < screen_count) {
    ...
    /* HACK! zmbvu_decode_frame() will check the necessary things!
    ...
    */
    //printf("%d\n", size);
    if (zmbvu_decode_frame(zc, packed, size) < 0) {
      printf("can't decode packed frame #%d\n", frameno);
      return 0;
    }
    memcpy(cur_pal, zmbvu_get_palette(zc), 768);
    for (int y = 0; y < 240; ++y) memcpy(cur_screen+y*320, zmbvu_get_decoded_line(zc, y), 320);
    ++frameno;
    return 1;
  }
  return 0;
...
static int do_decode_screens (int (*writer)(void *udata), void *udata) {
  int res = -1;
  zmbvu_unpacker_t zc;
  zc = zmbvu_unpacker_new();
  if (zc == NULL) { printf("FATAL: can't create decoder!\n"); return -1; }
  if (zmbvu_decode_setup(zc, 320, 240) < 0) { printf("FATAL: can't init decoder!\n"); goto quit; }
  frameno = 0;
